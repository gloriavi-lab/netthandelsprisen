...
# ─────────────────────────────────────────────
# SESSION STATE OG PERSISTENT LAGRING
# ─────────────────────────────────────────────
import json
import os
...
def lagre_resultater_lokalt(data: dict):
    """Lagrer resultater til JSON-fil på disk (persistent mellom sessions i Streamlit Cloud)."""
    try:
...
def last_resultater_lokalt() -> dict:
    """Laster resultater fra JSON-fil hvis den finnes."""
    try:
...
if "resultater" not in st.session_state:
    # Prøv å laste fra disk ved oppstart
    lagret = last_resultater_lokalt()
...
with st.sidebar:
    ...
    if st.session_state.resultater:
        ...
        st.download_button(
            "↓ Last ned resultater (JSON)",
            data=json_str.encode("utf-8"),
            file_name="netthandelsprisen_resultater.json",
